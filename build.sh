...
# Orlan Editor: our product name, and no connection or document limit. configure.ac defaults
# both limits to 9999 when no --with-welcome-url is given (which also keeps home mode, and its
# limit of 20 connections and 10 documents, out of the build). We set them higher still, so
# memory is the only limit. configure caps documents at connections, so both are the same.
ORLAN_LIMIT=100000
( cd online && ./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var --enable-silent-rules --disable-tests --with-lokit-path="$engine_dir"/include --with-lo-path=/opt/collaboraoffice --with-lo-builddir="$engine_dir" CPPFLAGS="$engine_abi_flag" \
    --with-app-name="Orlan Editor" --with-vendor="Orlan" --with-info-url="https://orlan.app" \
    --with-max-connections=$ORLAN_LIMIT --with-max-documents=$ORLAN_LIMIT \
    $ONLINE_EXTRA_BUILD_OPTIONS) || exit 1
# Proof in the build log: the limits the compiler sees.
grep -E '^#define (MAX_CONNECTIONS|MAX_DOCUMENTS|APP_NAME) ' online/config.h || exit 1
echo "Online source commit: $(cd online && git rev-parse HEAD)"
